import Link from 'next/link';
  ...
  let mugs={}
    data.forEach(item => {
      // console.log('mugs[item.title]',mugs[item.title])
      if (item.title in mugs) {
        if (item.availQty>0 && item.category=="mugs" && !mugs[item.title]["color"].includes(item.color)) {
          // console.log('mugs[item.title]["color"]',mugs[item.title]["color"],item.color);
          mugs[item.title]["color"].push(item.color)
        }
        if (item.availQty>0 && item.category=="mugs" && !mugs[item.title]["size"].includes(item.size)) {
          mugs[item.title]["size"].push(item.size)
        }
      } else {
        if (item.category=="mugs" && item.availQty>0 ) {
          mugs[item.title]={...item};  
